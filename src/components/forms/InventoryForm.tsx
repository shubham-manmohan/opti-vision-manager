...
import { InventoryItem, useAppStore } from '@/lib/store';
...
export const InventoryForm = ({ item, onClose, onSave }: InventoryFormProps) => {
  ...
  const calculateMargin = () => {
    if (formData.costPrice > 0 && formData.sellingPrice > 0) {
      return ((formData.sellingPrice - formData.costPrice) / formData.costPrice * 100).toFixed(1);
    }
    return '0';
  };
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  {errors.itemCode && <p className="text-sm text-destructive">{errors.itemCode}</p>}
                </div>
                ...
                <div className="p-3 bg-primary/5 rounded-lg border border-primary/20">
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="font-medium text-foreground">
                      Profit: ₹{(formData.sellingPrice - formData.costPrice).toLocaleString()}
                    </span>
            ...
            <div className="space-y-4">
              ...
              {formData.currentStock <= formData.reorderLevel && formData.currentStock > 0 && (
                <div className="p-3 bg-warning/10 rounded-lg border border-warning/20">
                  <p className="text-sm text-warning font-medium">
                    ⚠️ Low stock alert: Current stock ({formData.currentStock}) is at or below reorder level ({formData.reorderLevel})
                  </p>
                </div>
              )}
